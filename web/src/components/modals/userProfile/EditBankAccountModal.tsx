// components/modals/EditBankAccountModal.tsx
'use client';
...
export function EditBankAccountModal({
  ...
  const { updateBankAccount, isUpdating } = useProfileStore();
  ...
  useEffect(() => {
    if (isOpen && user && bankAccountId) {
      const bankAccounts = (user.onboarding?.bankDetails || []) as any[];
      const account = bankAccounts.find((acc: any) => acc.id === bankAccountId);
  ...
  }, [isOpen, user, bankAccountId, reset]);
